...
class Subcribe():
    ...
    """
    def __init__(self, app_client_id, app_client_secret, ws, **kwargs):
        """
        ...
        # self.c.bind(new_fac_data=self.on_new_fac_data)
        self.c.bind(new_met_data=self.on_new_met_data)
        self.c.bind(new_pow_data=self.on_new_pow_data)
        self.c.bind(inform_error=self.on_inform_error)
        self.ws = ws

    def start(self, streams, profile_name, headsetId=''):
        """
    ...
    def on_query_profile_done(self, *args, **kwargs):
        ...
        print('Saved profiles', self.profile_lists)
        if self.profile_name in self.profile_lists:
            # the profile is existed
            self.c.get_current_profile()
        else:
            print(self.profile_name, ' does not exist in saved profiles')
            return;
    ...
    def on_new_com_data(self, *args, **kwargs):
        ...
        data: dictionary
             The values in the array pow match the labels in the array labels return at on_new_data_labels
        For example: {'com': [5.251, 4.691, 3.195, 1.193, 0.282, 0.636, 0.929, 0.833, 0.347, 0.337, 7.863, 3.122, 2.243, 0.787, 0.496, 5.723, 2.87, 3.099, 0.91, 0.516, 5.783, 4.818, 2.393, 1.278, 0.213], 'time': 1627459390.1729}
        """
        data = kwargs.get('data')
        printSubData(self.ws, data)
        if data['action'] != 'neutral':
             print("User thinking of {} command".format(data['action']))
...
def on_open(ws):
     ...
     your_app_client_id = "cfywOsgESBT6Gn0LMeMKIkrMV7JC8P7T4xrVXy2a"
     your_app_client_secret = "lVEQChhNKjWk3xO3jBonzWDYiRJ9J1jgWVSNerq9Ja2fiCoA3U1SenyjpC84ehDiMzGiMTrCV92qlnXrmPh8t12irVpxgr8HUF9JzDpEsFqlQspKkKRM2y7kLXS47TsY"

     s = Subcribe(your_app_client_id, your_app_client_secret, uri = "wss://localhost:6868", ws=ws)

    # list data streams
     streams = ['com','fac']
     s.start(streams, 'Sadeeq')
